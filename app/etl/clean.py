...
def run_etl():
    ...
    with engine.connect() as conn:
        ...
        for i in ids:
            print(f"[aviation_license] id={i} → license_number et aircraft_concerned NULL")
        if ids:
        ...
        for row in rows:
            print(f"[payment_tracking] id={row.id} → {row.payment_mode} | amount = 0 | date={row.payment_date}")
        if ids:
        ...
        ids = [row.id for row in rows]
        for row in rows:
            print(f"[tax_obligation] id={row.id} → {row.tax_type} | amount = 0 | période={row.period_start}")
        if ids:
        ...
        ids = [row.id for row in rows]
        for row in rows:
            print(f"[training] id={row.id} → {row.training_type} | cost = 0 | date={row.training_date}")
        if ids:
        ...
        ids = [row.id for row in rows]
        for row in rows:
            print(f"[waste_management] id={row.id} → {row.site} | {row.waste_type} | {row.weight_kg}kg | {row.period_date}")
        if ids:
        ...
        ids = [row.id for row in rows]
        for row in rows:
            print(f"[co2_emission] id={row.id} → {row.route} | co2={row.co2_kg} | {row.period_date}")
        if ids:
        ...
        ids = [row.id for row in rows]
        for row in rows:
            print(f"[fuel_surcharge] id={row.id} → {row.route} | amount={row.amount_tnd} | {row.period_date}")
        if ids:
